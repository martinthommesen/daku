...
async function build(target: BuildTarget): Promise<boolean> {
  ...
  console.log(`[daku-dev] Building ${isMacOS ? "app bundle" : "app"}...`);
  if (!(await buildDaemon())) {
    console.error("[daku-dev] Daemon build failed; keeping the current app open.");
    return false;
  }
  const result = isMacOS
    ? await $`${join(root, "scripts/bundle.sh")} debug`.nothrow()
    : await $`cargo build --package daku --bin daku --bin daku`.nothrow();
  if (result.exitCode !== 0) {
    console.error("[daku-dev] Build failed; keeping the current app open.");
    return false;
  }
  return true;
}
...
async function buildDaemon(): Promise<boolean> {
  console.log("[daku-dev] Building daemon...");
  const result =
    await $`cargo build --package daku-daemon --features dev-binary --bin daku-debug-daemon`.nothrow();
  if (result.exitCode !== 0) {
    console.error("[daku-dev] Daemon build failed; keeping the current daemon running.");
    return false;
  }
  return true;
}
...
function launchApp(): ReturnType<typeof Bun.spawn> {
  console.log(`[daku-dev] Launching ${appPath}`);
  const command = isMacOS ? ["open", "-n", "-W", appPath] : [appPath];
  const launchedApp = Bun.spawn(command, {
    cwd: root,
    env: { ...process.env, DAKU_DAEMON_PATH: daemonPath },
    stdout: "inherit",
    stderr: "inherit",
  });
  void launchedApp.exited.then((exitCode) => {
    ...
    closeWatchers();
    clearRebuildTimer();
    console.log("[daku-dev] App exited; stopping the watcher.");
...
function reportWatcherError(error: Error): void {
  console.error("[daku-dev] File watcher failed:", error);
  process.exitCode = 1;
  void cleanup();
}
...
function targetForChange(directory: string, filename: string | Buffer | null): BuildTarget {
  ...
  if (
    relativePath.startsWith("daku-daemon/") ||
    relativePath.startsWith("daku-core/")
  ) {
    return "daemon";
  }
  return "app";
    ...
    while (queuedBuild !== undefined && !stopping) {
      ...
      if (target === "daemon") {
        if (daemonChangeRevision === buildDaemonRevision) {
          console.log(
            "[daku-dev] Daemon rebuilt; daku will swap the process without relaunching.",
          );
        }
        continue;
      }
      ...
      if (appChangeRevision !== buildAppRevision) {
        console.log(
          "[daku-dev] More changes arrived during the build; waiting to rebuild.",
        );
        continue;
      }
...
async function cleanup(): Promise<void> {
  if (stopping) return;
  stopping = true;
  console.log("[daku-dev] Stopping watcher and app...");
  closeWatchers();
  clearRebuildTimer();
  await stopApp();
}
...
if (appChangeRevision === initialAppRevision) {
  ...
  console.log(
    "[daku-dev] Changes arrived during the initial build; waiting to rebuild.",
  );
  if (queuedBuild !== undefined) void drainBuildQueue();
}

console.log(
  "[daku-dev] Watching for source changes. Daemon-only edits hot-reload without relaunching daku.",
);
